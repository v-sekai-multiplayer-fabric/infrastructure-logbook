...
- A **mirror** is exempt. Its README belongs to its upstream, and editing it forks a
  ...
  evidence: `datasource-foundationdb` and `idtx-flow` carry GitHub's fork flag. Owning the
  Windows builds of FoundationDB is not owning its code, so its README is not ours to cut.
